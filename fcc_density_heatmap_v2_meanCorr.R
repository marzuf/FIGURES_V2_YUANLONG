# Rscript fcc_density_heatmap_v2_meanCorr.R
...
outFolder <- "FCC_DENSITY_HEATMAP_V2_MEANCORR" 
...
inFolder <- "../FIGURES_V3_YUANLONG/RANDOM_FCC_AUC_RATIO_MEANCORRPERMUT_V2_EITHER"
...
if(buildData){
  ...
  all_result_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
    ...
    hicds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      inFile <- file.path(inFolder, hicds, exprds, "ds_all_permut.Rdata")
      inData <- get(load(inFile))
      all_fcc <- unlist(lapply(inData, function(x) x[["fcc_RorL"]]))
...
all_result_dt$hicds_lab <- "meanCorrPermut_RorL"
...
for(a_t in all_types) {
  ...
  save(sub_density_dt, file="sub_density_dt.Rdata", version=2)
